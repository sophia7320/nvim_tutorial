...
└── autocmds.md       # nvim_create_autocmd → augroup 深度解析（概念/三级对比/自查清单）→ LspAttach → ftplugin
```
...
| Augroup 是什么、为什么需要 | `autocmds.md` | §4.1 什么是 augroup |
| 不用/半用/正确用 augroup 的对比 | `autocmds.md` | §4.2 如果用 augroup 会怎样？（对比演示） |
| 辅助函数约定和命名规范 | `autocmds.md` | §4.3 实战：augroup 辅助函数 |
| augroup 在架构中的地位 | `autocmds.md` | §4.4 augroup 在配置架构中的地位 |
| 快速自查清单 | `autocmds.md` | §4.5 |
