...
opt.incsearch = true                   -- (19) 逐字符匹配时即时跳转
...
-- ═══════ 行末处理 ════════════════════════════════════════
opt.fixendofline = true                -- (40) 保存时自动在文件末尾加空行（POSIX 要求）
opt.endofline = true
...
opt.conceallevel = 0                   -- (43) 不隐藏 markdown 语法符号（如 ** 加粗标记）
...
opt.splitkeep = "screen"    -- (47) :split 后光标不跳
```
